...
async function loadCommits(
  ...
  const url = await addTokenToQuery(
      new URL(`${GITHUB_BASE_API}/repos/${fullName}/commits`)
  );
...
const CommitSection = ({date, commits, fullName}) => {
    ...
    <div>
      ...
      <ul style={styles.commitDay}>
        {commits.map(({commit}, index) => (
          <Commit key={index} commit={commit} fullName={fullName} />
        ))}
      </ul>
    </div>
  );
...
const CommitBox = (props) => {
  ...
  return (
    <InfiniteScroll
      pageStart={0}
      loadMore={(page) => {
        loadCommits(fullName, branchOrCommit, page, setCommits, setHasMore);
      }}
      hasMore={hasMore}
      loader={loader}
    >
